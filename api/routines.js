...
router.get('/', async (req, res) => {
  try {
    ...
    return res.send(publicRoutines);
  } catch (err) {
    console.log(err);
  }
});
...
router.patch('/:routineId', async (req, res) => {
  ...
  try {
    if (header && routineId) {
      ...
      if (verified.id != routine.creatorId) {
        res.status(403).send({
          message: `User ${verified.username} is not allowed to update ${routine.name}`,
          error: "I am error",
  ...
  try {
    ...
    if (header) {
      ...
      if (verified.id != routineId.id) {
        ...
        res.status(403).send({
          message: `User ${verified.username} is not allowed to delete ${routineId.name}`,
          error: "I am error",
          name: 'unauthorizedUser'
        })
      }
